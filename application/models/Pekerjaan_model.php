...
class Pekerjaan_model extends CI_Model {
	...
	public function get_job($param = FALSE){
		$hasil = $this->db->query("SELECT a.ID, a.PEKERJAAN, a.CATATAN, a.STATUS, a.INSERT_DATE, b.NAMA 
			FROM TB_PEKERJAAN a LEFT JOIN TB_USER b ON a.CREATED_BY = b.ID");
		return $hasil;
	}

	public function insert_job($data){
		$query = $this->db->insert('TB_PEKERJAAN', $data);
		return $query;
	}

	public function changestatus_pekerjaan($value, $id){
		$hasil = $this->db->query("UPDATE TB_PEKERJAAN SET STATUS = '".$value."' WHERE ID = '".$id."'");
		return $hasil;
	}
	...
	public function get_active_job(){
		$hasil = $this->db->query("SELECT ID, PEKERJAAN FROM TB_PEKERJAAN WHERE STATUS = '1'");
		return $hasil;
	}
	...
	public function get_justifikasi($id){
		$hasil = $this->db->query("SELECT a.ID, a.ID_TKI, a.ID_PEKERJAAN, a.ID_LOWONGAN, c.PEKERJAAN, a.STATUS, a.PROGRESS as 'PROGRESS_STAT',
			...
			a.DATA_DIRI_APPROVAL 'STATUS_DATA_DIRI_APPROVAL', (SELECT NAMA FROM TB_USER WHERE ID = a.APPROVED_BY_1) as 'APPROVED_BY_1', DATE_FORMAT(a.APPROVED_DATE_1, '%d %M %Y %H:%i') APPROVED_DATE_1, 
			...
			a.SURAT_PERJANJIAN_APPROVAL 'STATUS_SURAT_PERJANJIAN_APPROVAL', (SELECT NAMA FROM TB_USER WHERE ID = a.APPROVED_BY_2) as 'APPROVED_BY_2', DATE_FORMAT(APPROVED_DATE_2, '%d %M %Y %H:%i') APPROVED_DATE_2, 
			...
			a.KONTRAK_KERJA_APPROVAL 'STATUS_KONTRAK_KERJA_APPROVAL', (SELECT NAMA FROM TB_USER WHERE ID = a.APPROVED_BY_3) as 'APPROVED_BY_3', DATE_FORMAT(APPROVED_DATE_3, '%d %M %Y %H:%i') APPROVED_DATE_3, (SELECT NAMA FROM TB_USER WHERE ID = a.ASSIGNED_BY) as 'ASSIGNED_BY', DATE_FORMAT(ASSIGNED_DATE, '%d %M %Y %H:%i') ASSIGNED_DATE, a.ID_KONTRAK, a.ID_PERJANJIAN, a.CATATAN, a.CREATED_BY,
	...
	public function get_all_pekerjaan(){
		...
		$hasil = $this->db->query("SELECT *, CASE WHEN APPROVE = '0' AND STATUS_JUSTIFIKASI = '0' THEN '2' WHEN APPROVE = '1' AND STATUS_JUSTIFIKASI = '0' THEN '1' WHEN APPROVE = '1' AND STATUS_JUSTIFIKASI is null THEN '1' WHEN APPROVE = '0' AND STATUS_JUSTIFIKASI is null THEN '2' WHEN APPROVE = '2' THEN '2' ELSE '0' END AS APPROVAL FROM (SELECT a.ID 'ID_TKI', a.NIK, a.MAID_CODE, a.NAMA, a.APPROVE, b.ID 'ID_JUSTIFIKASI', b.STATUS 'STATUS_JUSTIFIKASI', b.PROGRESS, b.ID_LOWONGAN, b.SURAT_PERJANJIAN_APPROVAL, b.KONTRAK_KERJA_APPROVAL, (SELECT PEKERJAAN FROM TB_PEKERJAAN WHERE ID = b.ID_PEKERJAAN) as 'PEKERJAAN' FROM JUSTIFIKASI_PEKERJAAN b LEFT JOIN MST_TKI a ON a.ID = b.ID_TKI ".$condition.") AS ab");
		return $hasil;
	}
	...
	public function get_assign_pekerjaan($param, $id){
		if ($param == 'LISTASSIGN') {
			$query = "SELECT a.ID, a.JOB, b.PEKERJAAN, a.PENERIMA_JASA, c.NEGARA, a.SLOT_PEKERJAAN FROM TB_LOWONGAN a JOIN TB_PEKERJAAN b ON a.JENIS_PEKERJAAN = b.ID JOIN TB_NEGARA c ON a.NEGARA = c.CODE WHERE a.JENIS_PEKERJAAN = '".$id."' AND a.IS_USED = 'N'";
		} elseif($param == 'GETASSSIGNJOB') {
			$query = "SELECT a.ID, a.JOB, b.PEKERJAAN, a.PENERIMA_JASA, c.NEGARA FROM TB_LOWONGAN a JOIN TB_PEKERJAAN b ON a.JENIS_PEKERJAAN = b.ID JOIN TB_NEGARA c ON a.NEGARA = c.CODE WHERE a.ID = '".$id."'";
		}
		$hasil = $this->db->query($query);
		return $hasil;
	}
	...
	public function addRiwayatPekerjaan($id){
		$hasil = $this->db->query("INSERT INTO RIWAYAT_PEKERJAAN (ID, ID_TKI, ID_PEKERJAAN, LAMA_BEKERJA, SATUAN_LAMA_BEKERJA, LOKASI, CREATED_BY)
		  SELECT
		    ...
		    (SELECT LAMA_KONTRAK FROM TB_KONTRAK WHERE ID = a.ID_KONTRAK) AS LAMA_BEKERJA,
		    (SELECT SATUAN_LAMA_KONTRAK FROM TB_KONTRAK WHERE ID = a.ID_KONTRAK) AS SATUAN_LAMA_BEKERJA,
		    (SELECT NEGARA FROM TB_LOWONGAN WHERE ID = a.ID_LOWONGAN) AS LOKASI,
