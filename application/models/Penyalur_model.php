...
class Penyalur_model extends CI_Model {
	...
	public function get_all_approve(){
		$hasil = $this->db->query("SELECT a.ID, a.NAMA, a.NIK, a.APPROVE, 
			...
			END AS APPROVE_PROGRESS, c.NAMA_PERUSAHAAN FROM MST_PENYALUR a JOIN TB_GROUP b ON a.ID_GROUP = b.ID JOIN MST_PERUSAHAAN c 
	...
	public function get_all_penyalur(){
		$hasil = $this->db->query("SELECT a.ID, a.NAMA, a.NIK, a.APPROVE, 
			...
			WHEN APPROVE = '1' THEN 'APPROVED'
			WHEN APPROVE = '2' THEN 'REJECTED'
			END AS APPROVE_PROGRESS, c.NAMA_PERUSAHAAN, (SELECT USERNAME FROM TB_USER WHERE ID_PENYALUR = a.ID) as 'USERNAME' FROM MST_PENYALUR a JOIN TB_GROUP b ON a.ID_GROUP = b.ID JOIN MST_PERUSAHAAN c 
	...
	public function get_penyalur_detail($id){
		...
		CASE WHEN a.APPROVE = '0' THEN 'NEW'
			...
			END AS APPROVE_PROGRESS, a.INSERT_DATE, a.CATATAN, a.NIK, a.ALAMAT, a.TEMPAT_LAHIR, a.TANGGAL_LAHIR, b.KET, b.ID as 'ID_GROUP', c.NAMA_PERUSAHAAN, e.NAMA as 'NAMA_PENGAJU', (SELECT USERNAME FROM TB_USER WHERE ID_PENYALUR = a.ID) as 'USERNAME' FROM MST_PENYALUR a JOIN TB_GROUP b ON a.ID_GROUP = b.ID JOIN MST_PERUSAHAAN c ON b.ID_PERUSAHAAN = c.ID JOIN TB_USER e ON a.CREATED_BY = e.ID WHERE a.ID = '".$id."'");
		return $hasil;	
	}
